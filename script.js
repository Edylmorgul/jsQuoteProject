...
let citations = [
  ...
  let btnCitation = document.querySelector("#nouveau");
  let citation = document.querySelector("#citation");
  let auteur = document.querySelector("#auteur");
  let dernier = 0; // Dernier index choisi
  let index = 0;

  btnCitation.addEventListener("click", changeCitation);

  function changeCitation(){
      let newCitation;  
      newCitation = getNewCitation();

      citation.textContent = newCitation[0];
      auteur.textContent = newCitation[1];
  }

  function getNewCitation(){
      do{
      ...
      dernier = index; // On récup le dernier index valider
